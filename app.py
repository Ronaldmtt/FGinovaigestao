...
with app.app_context():
    ...
    try:
        db.session.execute(text("ALTER TABLE project_status_history ADD COLUMN note VARCHAR(255)"))
        db.session.commit()
        print("Migração Executada: note em project_status_history.")
    except Exception:
        db.session.rollback()
        pass
    ...
    crm2_lead_queries = [
        ...
        'ALTER TABLE crm2_leads ADD COLUMN external_payload_json TEXT',
        'ALTER TABLE crm2_leads ADD COLUMN last_site_sync_at TIMESTAMP'
    ]
    for q in crm2_lead_queries:
        try:
            db.session.execute(text(q))
            db.session.commit()
            print(f"Migracao Executada: {q}")
