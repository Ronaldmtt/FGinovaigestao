...
# The exact content we saw in view_file:
old_content_start = """@app.route('/reports/generate_pdf')
@login_required
def generate_pdf():
    # Filtros
    project_id = request.args.get('project_id')"""

new_content = """@app.route('/reports/generate_pdf', methods=['GET', 'POST'])
@login_required
def generate_pdf():
    projects = []
...
# Locate the start of the old function
start_idx = content.find(old_content_start)

if start_idx == -1:
    print("Could not find generate_pdf function start signature")
    # Debug: print first 50 chars of what we looked for
    print(f"Looked for: {old_content_start[:50]}...")
    exit(1)

# Since generate_pdf is at the end of the file (confirmed by view_file showing up to 4279), 
# checking if there's anything else after.
# Assuming it runs until the end of the file.
# We will replace from start_idx to the end.

print(f"Found function at index {start_idx}")
...
print("routes.py updated successfully via full content replace")
